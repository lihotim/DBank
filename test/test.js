  ...
  describe('testing withdraw...', () => {
    ...
    describe('success', () => {
      ...
      beforeEach(async () => {
        ...
        await wait(2) //accruing interest // Note: This is how we can perform 'time' stuff in Unit tests. Here, the user deposits 0.01 ETH for 2 seconds and then withdraws it.

        balance = await web3.eth.getBalance(user) // Note: it means the account balance of the user AFTER depositing ETH into the dbank smart contract. Say he has deposited 99 ether, balance should be 0.99xxxxxxxxxxxxxxx.
        await dbank.withdraw({from: user})
      })

      it('balances should decrease', async () => {
        expect(Number(await web3.eth.getBalance(dbank.address))).to.eq(0) // Note: As the user has already withdrew ALL his ETH from the dbank smart contract, there should now be 0 ETH in the dbank smart contract.
        expect(Number(await dbank.etherBalanceOf(user))).to.eq(0)
      })

      it('user should receive ether back', async () => {
        expect(Number(await web3.eth.getBalance(user))).to.be.above(Number(balance)) // Note: recall line 88, now the user has withdrew his 99 ETH back, now his account balance should be 99.99xxxxxxxxx.
      })
      ...
      it('user should receive proper amount of interest', async () => {
        ...
        balance = Number(await token.balanceOf(user)) //Note: this 'balance' means how much DBC token the user has earned as interest in 2 seconds.
